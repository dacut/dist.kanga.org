...
class QuerySignatures(TestCase):
    ...
    def verify(self, method, url, body, timestamp, headers, signed_headers,
        ...
        string_to_sign = (
            ...
            timestamp + "\n" +
            credential_scope + "\n" +
            sha256(canonical_req).hexdigest())

        def sign(secret, value):
            return hmac.new(secret, value, sha256).digest()

        k_date = sign("AWS4" + secret_key, date)
        k_region = sign(k_date, region)
        k_service = sign(k_region, service)
        k_signing = sign(k_service, "aws4_request")
        signature = hmac.new(k_signing, string_to_sign, sha256).hexdigest()
...
class BadTypeInitializer(TestCase):
    def runTest(self):
        ...
        for i in xrange(len(params)):
            if not isinstance(params[i], basestring):
                continue
